//! Runs `config/benchmarks/citation_integrity_v1.yaml` against
//! `armor_core::detectors::citation_integrity`. Formula-based (grounds
//! citation markers against caller-supplied `sources`), so this follows
//! `gibberish_benchmark.rs`'s shape rather than the rule-bank coverage
//! pattern — one rule, recall + zero-FP only.

use armor_core::detectors::citation_integrity;
use armor_core::policy::schema::CheckOptions;
use serde::Deserialize;

fn opts(sources: &[String]) -> CheckOptions {
    let refs: Vec<&str> = sources.iter().map(String::as_str).collect();
    let mut o = CheckOptions::default();
    o.set_str_list("sources", &refs);
    o
}
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/citation_integrity_v1.yaml"
    ))
    .expect("config/benchmarks/citation_integrity_v1.yaml must parse")
}
...
fn recall_is_100_percent_on_positive_samples() {
    ...
    for sample in &corpus.positive_samples {
        let result = citation_integrity::evaluate(&sample.text, &opts(&sample.sources));

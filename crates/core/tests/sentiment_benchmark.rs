//! Runs `config/benchmarks/sentiment_v1.yaml` against
//! `armor_core::detectors::sentiment` and asserts the acceptance criteria
//! documented in that file's header.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::sentiment;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!("../../../config/benchmarks/sentiment_v1.yaml"))
        .expect("config/benchmarks/sentiment_v1.yaml must parse")
}

fn hit_rule_ids(text: &str) -> HashSet<String> {
    sentiment::evaluate(text, &CheckOptions::default())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()
...
fn rule_ids() -> Vec<String> {
    let rules: Vec<RawRule> =
        serde_yaml::from_str(include_str!("../../../rules/sentiment/rules.yaml")).unwrap();

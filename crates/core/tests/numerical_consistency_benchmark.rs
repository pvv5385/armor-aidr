//! Runs `config/benchmarks/numerical_consistency_v1.yaml` against
//! `armor_core::detectors::numerical_consistency`. Pure-computation base
//! tier (no rules.yaml bank), so this follows `gibberish_benchmark.rs`'s
//! shape — one rule, recall + zero-FP only.

use armor_core::detectors::numerical_consistency;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/numerical_consistency_v1.yaml"
    ))
    .expect("config/benchmarks/numerical_consistency_v1.yaml must parse")
}
...
fn recall_is_100_percent_on_positive_samples() {
    ...
    for sample in &corpus.positive_samples {
        let result = numerical_consistency::evaluate(&sample.text, &CheckOptions::default());

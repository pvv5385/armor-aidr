//! Runs `config/benchmarks/hallucination_v1.yaml` against
//! `armor_core::detectors::hallucination`. Base-tier token-overlap grounding
//! (no rules.yaml bank), so this follows `gibberish_benchmark.rs`'s shape —
//! one rule, recall + zero-FP only.

use armor_core::detectors::hallucination;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/hallucination_v1.yaml"
    ))
    .expect("config/benchmarks/hallucination_v1.yaml must parse")
}
...
fn recall_is_100_percent_on_positive_samples() {
    ...
    for sample in &corpus.positive_samples {
        let result = hallucination::evaluate(&sample.text, &opts(&sample.sources));

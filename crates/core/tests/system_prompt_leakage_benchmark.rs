//! Runs `config/benchmarks/system_prompt_leakage_v1.yaml` against
//! `armor_core::detectors::system_prompt_leakage`. Fuzzy pre-filter only,
//! word-shingle Jaccard similarity (no rules.yaml bank), so this follows
//! `gibberish_benchmark.rs`'s shape — one rule, recall + zero-FP only.

use armor_core::detectors::system_prompt_leakage;
use armor_core::policy::schema::CheckOptions;
use serde::Deserialize;

fn opts(prompts: &[String]) -> CheckOptions {
    let refs: Vec<&str> = prompts.iter().map(String::as_str).collect();
    let mut o = CheckOptions::default();
    o.set_str_list("system_prompts", &refs);
    o
}
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/system_prompt_leakage_v1.yaml"
    ))
    .expect("config/benchmarks/system_prompt_leakage_v1.yaml must parse")
}
...
fn recall_is_100_percent_on_positive_samples() {
    ...
    for sample in &corpus.positive_samples {
        let result = system_prompt_leakage::evaluate(&sample.text, &opts(&sample.system_prompts));

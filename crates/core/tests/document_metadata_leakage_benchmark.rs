//! Runs `config/benchmarks/document_metadata_leakage_v1.yaml` against
//! `armor_core::detectors::document_metadata_leakage` and asserts the
//! acceptance criteria documented in that file's header.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::document_metadata_leakage;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/document_metadata_leakage_v1.yaml"
    ))
    .expect("config/benchmarks/document_metadata_leakage_v1.yaml must parse")
}

fn hit_rule_ids(text: &str) -> HashSet<String> {
    document_metadata_leakage::evaluate(text, &CheckOptions::default())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()
...
/// Only the regex-driven rules from `rules.yaml` — `hidden-invisible-text`
/// is a code-only check with no entry there, so it's exempt from this
/// per-rule coverage requirement (recall/FPR tests still cover it, since
/// its samples carry that `rule_id` in the corpus).

//! Runs `config/benchmarks/structure_validation_v1.yaml` against
//! `armor_core::detectors::structure_validation` with `require_valid_json:
//! true, max_bytes: 200, max_depth: 4`.

use armor_core::detectors::structure_validation;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/structure_validation_v1.yaml"
    ))
    .expect("config/benchmarks/structure_validation_v1.yaml must parse")
}

fn opts() -> CheckOptions {
    let mut o = CheckOptions::default();
    o.set_bool("require_valid_json", true);
    o.set_f64("max_bytes", 200.0);
    o.set_f64("max_depth", 4.0);
    o
}
...
fn recall_is_100_percent_on_positive_samples() {
    ...
    for sample in &corpus.positive_samples {
        let result = structure_validation::evaluate(&sample.text, &opts());

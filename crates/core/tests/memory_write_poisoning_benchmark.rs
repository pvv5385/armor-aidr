//! Runs `config/benchmarks/memory_write_poisoning_v1.yaml` against
//! `armor_core::detectors::memory_write_poisoning` and asserts the
//! acceptance criteria documented in that file's header.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::memory_write_poisoning;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/memory_write_poisoning_v1.yaml"
    ))
    .expect("config/benchmarks/memory_write_poisoning_v1.yaml must parse")
}

fn hit_rule_ids(text: &str) -> HashSet<String> {
    memory_write_poisoning::evaluate(text, &CheckOptions::default())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()

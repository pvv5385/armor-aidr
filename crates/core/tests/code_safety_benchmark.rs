//! Runs `config/benchmarks/code_safety_v1.yaml` against
//! `armor_core::detectors::code_safety` and asserts the acceptance criteria
//! documented in that file's header.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::code_safety;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/code_safety_v1.yaml"
    ))
    .expect("config/benchmarks/code_safety_v1.yaml must parse")
}

fn hit_rule_ids(text: &str) -> HashSet<String> {
    code_safety::evaluate(text, &CheckOptions::default())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()
...
fn rule_ids() -> Vec<String> {
    let rules: Vec<RawRule> =
        serde_yaml::from_str(include_str!("../../../rules/code_safety/rules.yaml")).unwrap();

//! Runs `config/benchmarks/pci_v1.yaml` against
//! `armor_core::detectors::pci` with `credit_card`/`luhn_required` enabled.

use armor_core::detectors::pci;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!("../../../config/benchmarks/pci_v1.yaml"))
        .expect("config/benchmarks/pci_v1.yaml must parse")
}
...
fn recall_is_100_percent_on_positive_samples() {
    ...
    for sample in &corpus.positive_samples {
        let result = pci::evaluate(&sample.text, &opts());

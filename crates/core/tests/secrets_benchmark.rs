//! Runs `config/benchmarks/secrets_v1.yaml` against
//! `armor_core::detectors::secrets`. See that file's header — this is a
//! REPRESENTATIVE 17-of-57-provider pass, not exhaustive; the coverage
//! check below validates internal consistency of the samples actually
//! present, not full `rules/secrets/rules.yaml` enumeration.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::secrets;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!("../../../config/benchmarks/secrets_v1.yaml"))
        .expect("config/benchmarks/secrets_v1.yaml must parse")
}

fn all_providers_enabled(providers: &[String]) -> CheckOptions {
...
fn recall_is_100_percent_on_positive_samples() {
    let corpus = corpus();
    let options = all_providers_enabled(&corpus.providers_covered);

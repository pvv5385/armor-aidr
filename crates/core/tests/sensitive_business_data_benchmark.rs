//! Runs `config/benchmarks/sensitive_business_data_v1.yaml` against
//! `armor_core::detectors::sensitive_business_data` and asserts the
//! acceptance criteria documented in that file's header.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::sensitive_business_data;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/sensitive_business_data_v1.yaml"
    ))
    .expect("config/benchmarks/sensitive_business_data_v1.yaml must parse")
}

fn hit_rule_ids(text: &str) -> HashSet<String> {
    sensitive_business_data::evaluate(text, &CheckOptions::default())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()

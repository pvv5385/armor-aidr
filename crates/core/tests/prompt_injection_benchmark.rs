//! Runs `config/benchmarks/prompt_injection_v1.yaml` against
//! `armor_core::detectors::prompt_injection`. See that file's header — a
//! REPRESENTATIVE 17-of-29-rule pass, not exhaustive; the coverage check
//! validates internal consistency of the samples present, not full
//! `rules/prompt_injection/rules.yaml` enumeration.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::prompt_injection;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/prompt_injection_v1.yaml"
    ))
    .expect("config/benchmarks/prompt_injection_v1.yaml must parse")
}

fn hit_rule_ids(text: &str) -> HashSet<String> {
    prompt_injection::evaluate(text, &CheckOptions::default())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()

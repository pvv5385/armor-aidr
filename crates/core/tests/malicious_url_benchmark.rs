//! Runs `config/benchmarks/malicious_url_v1.yaml` against
//! `armor_core::detectors::malicious_url` with every mechanism enabled.

use std::collections::{HashMap, HashSet};

use armor_core::detectors::malicious_url;
...
fn corpus() -> Corpus {
    serde_yaml::from_str(include_str!(
        "../../../config/benchmarks/malicious_url_v1.yaml"
    ))
    .expect("config/benchmarks/malicious_url_v1.yaml must parse")
}
...
fn hit_rule_ids(text: &str) -> HashSet<String> {
    malicious_url::evaluate(text, &all_enabled())
        .hits
        .into_iter()
        .map(|h| h.rule_id)
        .collect()
